...
def install_os_specific_dependencies_apt():
	irods_python_ci_utilities.install_os_packages(['make', 'gcc', 'bison', 'flex'])

def install_os_specific_dependencies_yum():
	# Delete the centos 7 check when centos 7 is no longer supported (June 30, 2024)
	import distro
	if distro.id() == 'centos' and distro.major_version() == '7':
		print('Configuring CERT Forensics Tools repository for flex 2.6.1. This only applies for Centos 7. '
			  'See https://forensics.cert.org/#centossupport')
		irods_python_ci_utilities.subprocess_get_output(
			['sudo', 'yum', '-y', 'install', 'centos-release-scl-rh'], check_rc=True)
		irods_python_ci_utilities.subprocess_get_output(
		...
		irods_python_ci_utilities.subprocess_get_output(
			...
			check_rc=True)
		irods_python_ci_utilities.subprocess_get_output(
			['sudo', 'rpm', '-i', './cert-forensics-tools-release-el7.rpm'],
			check_rc=True)
...
def main(build_directory, output_root_directory, irods_packages_root_directory, externals_directory):
	...
	build_directory = os.path.abspath(build_directory or tempfile.mkdtemp(prefix='irods_genquery2_plugin_build_directory'))
