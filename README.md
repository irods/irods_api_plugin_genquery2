...
Pass the name of a federated zone as the first argument to run the query in that federated zone.
```
iquery <federated_zone> <query_string>
```

Pass `sql-only` as the first argument to view the SQL generated by the parser.
```
iquery sql-only <query_string>
```

_The generated SQL is never executed._
